...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int x = 0; x < boardSize; x++) {
            for (int y = 0; y < boardSize; y++) {
                ...
                if (currentTile != null) {
                    if (x > 0 && currentTile.value() == board.tile(x - 1, y).value()) {
                        return true;
                    }
                    if (x < boardSize - 1 && currentTile.value() == board.tile(x + 1, y).value()) {
                        return true;
                    }
                    if (y > 0 && currentTile.value() == board.tile(x, y - 1).value()) {
                        return true;
                    }
                    if (y < boardSize - 1 && currentTile.value() == board.tile(x, y + 1).value()) {
                        return true;
                    }
                }
            }
        ...
        for (int y = board.size() - 1; y >= 0; y--) {
            if (board.tile(x, y) != null) {
                ...
                while (targetY < board.size() - 1) {
                    ...
                    if (nextTile == null) {
                        targetY++;
                    } else if (nextTile.value() == tileValue && !nextTile.wasMerged()) {
                ...
                if (targetY != y) {
                    board.move(x, targetY, currentTile);
                }
            }
        }
    }
    ...
    public void tilt(Side side) {
        ...
        System.out.println();
        System.out.println("Tilting completed for side: " + side);
        System.out.println("Final board state after tilting:");
